...
const EDSS_CALCULATOR_URL = 'https://medicpulse.org/fr/calculators/edss';
...
export default function ConsultationScreen({ route, navigation }: any) {
  ...
  const [relapseDetails, setRelapseDetails] = useState('');
  ...
  const openEdssCalculator = () => {
    Linking.openURL(EDSS_CALCULATOR_URL).catch(() =>
      showAlert('Erreur', "Impossible d'ouvrir le calculateur EDSS.")
    );
  };

  const goNext = () => {
    if (!patient?.medical_record_number) return showAlert('Numéro de dossier manquant', "Ce patient n'a pas encore de numéro de dossier. Ajoutez-le d'abord dans sa fiche patient.");
    if (!visitDate.trim()) return showAlert('Champ requis', 'Renseignez la date de visite.');
  ...
  const save = async () => {
    ...
    if (!patient?.medical_record_number || !isoVisitDate) return goNext();
    if (csfProfile && !['1', '2', '3', '4'].includes(csfProfile)) return showAlert('Profil LCR invalide', 'Choisissez un profil de type 1, 2, 3 ou 4.');
    if (hptDominance && !['dominant', 'non_dominant'].includes(hptDominance)) return showAlert('9HPT invalide', 'Choisissez dominant ou non-dominant.');
    setLoading(true);
    const payload = {
      patient_id: patient.id, medecin_id: session?.user.id, medical_record_number: patient.medical_record_number, visit_date: isoVisitDate,
      relapse: toBoolean(relapse), relapse_details: relapse === 'yes' ? textOrNull(relapseDetails) : null, weight_kg: numberOrNull(weight), height_cm: numberOrNull(height), physical_exam: textOrNull(physicalExam), edss_score: numberOrNull(edss),
  ...
  return <View style={styles.container}><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled"><Text style={styles.title}>Ajouter une consultation</Text><Text style={styles.patient}>{patient.first_name} {patient.last_name}</Text><Text style={styles.recordNumber}>Dossier n° {patient.medical_record_number || '—'}</Text><View style={styles.stepper}><Text style={[styles.step, page === 1 && styles.stepActive]}>1. Visite</Text><Text style={styles.separator}>—</Text><Text style={[styles.step, page === 2 && styles.stepActive]}>2. Examen</Text></View>
    {page === 1 ? <>
      ...
      {relapse === 'yes' && field('Préciser', relapseDetails, setRelapseDetails, 'default', true)}
      ...
      <TouchableOpacity style={styles.edssLink} onPress={openEdssCalculator}><Text style={styles.edssLinkText}>🔗 Calculer le score EDSS en ligne</Text></TouchableOpacity>
      ...
      {field('Traitement reçu', treatment, setTreatment, 'default', true)}
    </>}
